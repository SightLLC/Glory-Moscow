...
function custom_avatar_defaults ( $avatar_defaults ) {
    $x = rand(1, 43);
    $avatar_url = 'http://glorymoscow.ru/img/avatar/'.$x.'.png';
    $avatar_defaults[$avatar_url] = __( 'Custom Default Avatar', 'mytextdomain' );

    return $avatar_defaults;
}
function user_avatar ($id, $size) {
            ...
            $query = "SELECT avatar_id FROM profiles WHERE user_id=".$id;
            $res = mysql_query($query);
            $ava = mysql_fetch_row($res);
            echo '<img src="http://glorymoscow.ru/img/avatar/'.$ava[0].'.png" height="'.$size.'px" width="'.$size.'px" >';
}
